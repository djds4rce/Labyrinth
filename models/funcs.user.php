<?php
	...
	*/
	...
	function updateLastActivationRequest($new_activation_token,$username,$email)
	{
        ...
        //function checks if the user score is sufficent enough for the user to view the level
          function isscoresufficient($value)
        { 
            global $loggedInUser,$db,$db_table_prefix;
           $query="SELECT score FROM `".$db_table_prefix."Users` 
                WHERE 
                   User_ID = '".$db->sql_escape($loggedInUser->user_id)."'";
        ...
        function updatescore($value)
        {
          ...
          if($score==$value)
          {
             $value=$value+1;//bad way of coding i don know how to use PHPvaribale+1 in DB
